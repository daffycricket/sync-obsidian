...
| # | Priorité | Action | Description | Impacts | Rétrocompatible | Dépendances additionnelles | Tests de validation et de non regression | Commentaires |
|---|----------|--------|-------------|---------|-----------------|---------------------------|------------------------------------------|--------------|
| f1 | **P3 - BASSE** | **Toggle visibilité mot de passe** | Ajouter une icône "œil" pour afficher/masquer le mot de passe dans les settings | Meilleure UX : vérifier sa saisie sans erreur | ✅ Oui | Aucune | Test manuel : toggle eye icon dans settings, vérifier affichage/masquage | Modifier `settings.ts` avec `setIcon("eye")` / `setIcon("eye-off")` |

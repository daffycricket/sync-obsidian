...
class TestTimezoneHandling:
    """Tests pour la gestion des timezones dans les comparaisons de dates."""
    
    @pytest.mark.asyncio
    async def test_sync_with_utc_timezone_aware_dates(self, authenticated_client):
        """
        Test du scénario second laptop : dates avec timezone Z (UTC).
        Le client envoie des dates au format ISO avec Z, le serveur stocke en naive.
        La comparaison doit fonctionner sans erreur.
        """
        client, token = authenticated_client
        
        # Laptop 1 crée une note (stockée en UTC naive dans SQLite)
        await client.post(
            "/sync/push",
            headers=auth_headers(token),
            json={
                "notes": [{
                    "path": "timezone-test.md",
                    "content": "# Test timezone",
                    "content_hash": "hash1",
                    "modified_at": "2026-01-19T10:00:00",  # Sans Z (naive)
                    "is_deleted": False
                }]
            }
        )
        
        # Laptop 2 sync avec la même note mais date au format ISO avec Z (aware)
        response = await client.post(
            "/sync",
            headers=auth_headers(token),
            json={
                ...
                "notes": [{
                    "path": "timezone-test.md",
                    "content_hash": "hash2",  # Hash différent
                    "modified_at": "2026-01-19T12:00:00Z",  # Avec Z (UTC aware)
                    "is_deleted": False
                }],
                "attachments": []
            }
        ...
        # Doit retourner 200, pas 500 (TypeError)
        assert response.status_code == 200
        data = response.json()
        # Client plus récent (12:00 > 10:00) → push demandé
        assert "timezone-test.md" in data["notes_to_push"]
    
    @pytest.mark.asyncio
    async def test_sync_with_positive_timezone_offset(self, authenticated_client):
        """
        Test avec timezone offset positif (+01:00, +02:00, etc).
        """
        client, token = authenticated_client
        ...
        # Client sync avec timezone +02:00 (Europe/Paris en été)
        # 12:00+02:00 = 10:00 UTC, donc même heure que le serveur
        response = await client.post(
            "/sync",
            headers=auth_headers(token),
            json={
                ...
                "notes": [{
                    ...
                    "modified_at": "2026-01-19T12:00:00+02:00",  # +02:00 offset
                    "is_deleted": False
                }],
                "attachments": []
            }
        ...
        assert response.status_code == 200
        data = response.json()
        # 12:00+02:00 = 10:00 UTC = même heure serveur, hash différent → conflit
        assert len(data["conflicts"]) == 1
        assert data["conflicts"][0]["path"] == "tz-offset-test.md"
    
    @pytest.mark.asyncio
    async def test_second_laptop_scenario(self, authenticated_client):
        """
        Reproduit exactement le bug du second laptop :
        1. Laptop 1 push des notes
        2. Laptop 2 fait un premier sync (notes inconnues → push)
        3. Laptop 2 push ses notes
        4. Laptop 2 re-sync → comparaison timezone-aware vs naive
        """
        client, token = authenticated_client
        
        # Étape 1 : Laptop 1 crée des notes
        await client.post(
            "/sync/push",
            headers=auth_headers(token),
            json={
                "notes": [{
                    "path": "shared-note.md",
                    "content": "# Créé par Laptop 1",
                    "content_hash": "laptop1-hash",
                    "modified_at": "2026-01-19T10:00:00",
                    "is_deleted": False
                }]
            }
        ...
        # Étape 2 : Laptop 2 premier sync (a la même note localement)
        response1 = await client.post(
            ...
            json={
                ...
                "notes": [{
                    "path": "shared-note.md",
                    "content_hash": "laptop2-hash",
                    "modified_at": "2026-01-19T09:00:00Z",  # Avec Z, plus ancien
                    "is_deleted": False
                }],
                "attachments": []
            }
        ...
        # Étape 3 : Laptop 2 re-sync immédiat (le scénario qui crashait)
        response2 = await client.post(
            "/sync",
            headers=auth_headers(token),
            json={
                "last_sync": "2026-01-19T09:30:00Z",
                "notes": [{
                    "path": "shared-note.md",
                    "content_hash": "laptop2-hash",
                    "modified_at": "2026-01-19T09:00:00Z",
                    "is_deleted": False
                }],
                "attachments": []
            }
